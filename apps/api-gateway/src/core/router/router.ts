...
import votingRoomRouter from "@/features/voting-rooms/routes/voting_room.router.js";
...
export default (container: IDependencyInjection): Router => {
  ...
  ApplicationRouter.use("/profile", authenticateToken(container.tokenService), profileRouter(container));
  ApplicationRouter.use("/voting-room", authenticateToken(container.tokenService), votingRoomRouter(container));
